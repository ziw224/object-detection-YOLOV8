...
def predict_future_position(trajectory, future_frames=10):
    ...
    # Make sure the trajectory is used to forecast future positions properly
    past_positions = np.array(trajectory).reshape(-1, 2)
    times = np.arange(len(past_positions)).reshape(-1, 1)

    # Fit the linear regression model for x and y separately
    model_x = LinearRegression().fit(times, past_positions[:, 0])
    model_y = LinearRegression().fit(times, past_positions[:, 1])

    # Predict future position for both x and y
    future_time = np.array([len(times) + future_frames]).reshape(-1, 1)
...
def draw_boxes(img, bbox, names, object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        # Create new buffer for a new object
        if id not in data_deque:
            data_deque[id] = deque(maxlen=64)  # Assuming 64 is the trails length
        ...
        for j in range(1, len(data_deque[id])):
            if data_deque[id][j - 1] is None or data_deque[id][j] is None:
                continue
            thickness = int(np.sqrt(64 / float(j + j)) * 2.5)  # Adjust thickness to your preference
